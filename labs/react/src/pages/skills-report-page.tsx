...
export function SkillsReportPage() {
  ...
  return <section className="page graduation-page">
    <header className="report-hero"><div><span className="eyebrow"><BarChart3/> Relatório de competências</span><h1>Seu domínio React, baseado em evidências.</h1><p>Uma visão consolidada do estudo, prática, avaliação e entrega de produto.</p></div><div className="report-score" style={{'--score':score} as CSSProperties}><span><strong>{score}</strong><small>/100</small></span></div><aside><small>NÍVEL ATUAL</small><strong>{level}</strong><span>{completed}/{metrics.length} gates completos</span></aside></header>
