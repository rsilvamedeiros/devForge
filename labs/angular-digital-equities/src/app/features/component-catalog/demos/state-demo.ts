...
@Component({
  ...
  template: `
    ...
    <div class="demo-stage">
      @if (state() === 'loading') {
        <div class="demo-state"><mat-spinner diameter="28" /><span>Carregando ativos...</span></div>
      } @else if (state() === 'error') {
        <div class="demo-state is-error">
          <span>Falha ao carregar ativos.</span>
          <button mat-flat-button color="primary" type="button" (click)="state.set('ready')">Tentar novamente</button>
        </div>
      } @else if (state() === 'empty') {
        <div class="demo-state"><span>Nenhum ativo encontrado para esse filtro.</span></div>
      } @else {
        <ul class="demo-rows">
  ...
  styles: `
    ...
    .demo-actions .is-active { border-color: var(--trade-primary); color: var(--trade-primary); }
    .demo-stage { min-height: 116px; padding: 1rem; border: 1px solid var(--trade-border); border-radius: 11px; background: var(--trade-bg); }
    .demo-state { display: flex; align-items: center; gap: .75rem; color: var(--trade-muted); font-size: .85rem; }
    .demo-state.is-error { color: var(--trade-negative); }
